...
if( !empty($_GET['limit']) ) {
	...
	try {
		...
		if( !empty($limit) ) {
			...
			$stmt = $pdo->prepare("SELECT * FROM spotif ORDER BY id ASC");
		}
	...
	if( !empty($message_array) ) {
		...
		// 1行目のラベル作成
		$csv_data .= '"ID","曲","アーティスト","メッセージ","投稿日時"'."\n";
		
		foreach( $message_array as $value ) {
		
			// データを1行ずつCSVファイルに書き込む
			$csv_data .= '"' . $value['id'] . '","' .$value['track_name'] . '","' .  $value['artist_name'] . '","' .$value['message'].'","'. $value['post_date'] . "\"\n";
		}
	}
    	// ファイルを出力	
	echo $csv_data;
